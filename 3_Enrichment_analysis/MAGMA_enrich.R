#### MAGMA enrich ####
MAGMA_enrich = function(Net,Net_name,genemap,target.directory,output.directory,pathology,magma.directory = NULL){
  ...
  furrr::future_walk(universe,~{
    ...
    purrr::walk(patho.file,~{
      ...
      gene_set = paste0(output.directory,Net_name,"_gene.set") ### gene set info file created previously
      gene_include = paste0(output.directory,biotype) ### universe file previously created
      out = paste0(output.directory,"MAGMA/",Net_name,"_",bio,"_",annot.type,"_gene-level.analysis")
  ...
  result.name = grep(".gsa.out",list.files(paste0(output.directory,"MAGMA/")),value = T)
  ...
  PGC_MAGMA_out = sapply(universe,function(biotype){
    ...
    res = purrr::map(result,function(x){
      ...
      ### load output files
      PGC_out <- read.table(paste0(output.directory,"MAGMA/",x), 
  ...
  ### save ###
  save(PGC_MAGMA_out,file = paste0(output.directory,"MAGMA/",Net_name,"_",pathology,".RData"))
  
  ##### plot ####
  require(grDevices)
  purrr::iwalk(PGC_MAGMA_out,function(x,y){
    pdf(paste0(output.directory,"MAGMA/",Net_name,"_",y,"_",pathology,"_MAGMA.pdf"),width = 18, useDingbats = FALSE)
